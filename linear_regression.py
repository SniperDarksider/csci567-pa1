...
def regularized_linear_regression(X, y, lambd):
    ...
    #####################################################
    D = X.shape[1]
    X_ = np.linalg.inv(X.T.dot(X)+lambd*np.eye(D))
    w=X_.dot(X.T).dot(y)
    return w
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
    ...
    #				 YOUR CODE HERE					                    #
    bestlambda=0
    min_validation_error=1e9
    for lambd in lambds:
        w_star = regularized_linear_regression(Xtrain,ytrain,lambd)
        validation_error=test_error(w_star,Xval,yval)
...
def test_error(w, X, y):
    ...
    """
    err = np.mean((X.dot(w)-y).T.dot(X.dot(w)-y))
    return err
