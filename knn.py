...
def compute_distances(Xtrain, X):
    ...
    for i in range(num_test):
        for j in range(num_train):
            dist = np.sqrt(np.sum((X[i] - Xtrain[j]).T.dot(X[i] - Xtrain[j])))
            dists[i][j] = dist
    #####################################################
    return dists
...
def predict_labels(k, ytrain, dists):
    ...
    for i in range(num_test):
        top_k_index = dists[i].argsort()[:k][::1]
        cnt = 0
        val = -1
        for t in top_k_index:
            if val==-1:
                cnt+=1
                val=t
            elif t == val:
...
def compute_accuracy(y, ypred):
    ...
    #				 YOUR CODE HERE					                    #
    num_test = y.shape[0]
    acc= np.sum( y == ypred)*1.0/num_test
    #####################################################
    return acc
...
def find_best_k(K, ytrain, dists, yval):
    ...
    validation_accuracy=[]
    best_k=0
    max_validation_accuray=0
    for k in K:
        ypred=predict_labels(k,ytrain,dists)
        vac=compute_accuracy(yval,ypred)
        validation_accuracy.append(vac)
        if vac>max_validation_accuray:
            max_validation_accuray=vac
            best_k=k
    #####################################################
    #				 YOUR CODE HERE					                    #
    #####################################################
